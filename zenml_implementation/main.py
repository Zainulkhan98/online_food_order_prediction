...
#         categorical_cols = ['Gender', 'Marital Status', 'Family size', 'Feedback', 'Occupation', 'Monthly Income', 'Pin code', 'longitude']
#         data[categorical_cols] = pd.DataFrame(ohe.fit_transform(data[categorical_cols]))
#
#         le = LabelEncoder()
#         data['Output'] = le.fit_transform(data['Output'])
#     except Exception as e:  # Catch potential errors in preprocessing
#         print(f"Error during preprocessing: {e}")
#         return data  # Return the original data in case of errors
#
#     return data
#
#
# @step
# def split(data, test_size, target_column) -> Union[pd.DataFrame, pd.DataFrame, pd.Series, pd.Series]:
#     test_size = test_size
#     x = data.drop([target_column], axis=1)
#     y = data[target_column]
#     return train_test_split(x, y, test_size, random_state=42)
#
# @step
# def modeling(x_train: pd.DataFrame, y_train: pd.Series) -> LinearRegression:
...
#
# @step
# def predict(model: LinearRegression, x_test: pd.DataFrame) -> pd.Series:
...
#     x_train, x_test, y_train, y_test = split(processed_data, test_size=0.2, target_column = 'Output')  # Unpack directly
#     print('modeling data...')
